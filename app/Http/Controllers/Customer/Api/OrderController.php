...
//            ];
    ...
    public function cancelOrder(Request $request, $id){
        ...
        $message='Your order of Rs. '.($order->total_cost-$order->coupon_discount+$order->delivery_charge+$order->extra_amount).' at Vegans Fresh is cancelled. Order Reference ID: '.$order->refid;
        ...
        if(!empty($order->customer->mobile)){
            Msg91::send($order->customer->mobile, 'Order ID '.$order->refid.' at Vegans Fresh has been cancelled.', env('HALLO_CANCEL_ORDER_CUSTOMER'));
        }
...
//        }, $order['details']);
    ...
    public function raiseReturn(Request $request, $detail_id){
        ...
        if(isset($return->order->customer->mobile))
            Msg91::send($detail->order->customer->mobile, 'Return has been raised at Vegans Fresh for Order ID: '.$detail->order->refid.', Product: '.($detail->entity->name??'').', Quantity: '.$request->quantity, env('HALLO_RETURN_RAISED'));
