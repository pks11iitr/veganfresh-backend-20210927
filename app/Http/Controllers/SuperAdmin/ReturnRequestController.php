...
//    }
    ...
    public function cancelReturnRequest(Request $request,$return_id){
        ...
        if(isset($return->order->customer->mobile))
            Msg91::send($return->order->customer->mobile, 'Return at Vegans Fresh has been rejected for Order ID:'.$return->order->refid.', Product: '.($return->details->entity->name??''), env('HALLO_RETURN_REQUEST_REJECTED'));

        //send customer notification
        if(isset($return->order->storename->mobile))
            Msg91::send($return->order->storename->mobile, 'Return at Vegans Fresh has been rejected for Order ID: '.$return->order->refid.', Product: '.($return->details->entity->name??''), env('HALLO_RETURN_REQUEST_REJECTED'));

        if(isset($return->order->rider->mobile))
            Msg91::send($return->order->rider->mobile, 'Return at Vegans Fresh has been rejected for Order ID: '.$return->order->refid.', Product: '.($return->details->entity->name??''), env('HALLO_RETURN_REQUEST_REJECTED'));
    ...
    public function approveReturnProduct(Request $request, $return_id){
        ...
        //send customer notification
        if(isset($return->order->customer->mobile))
            Msg91::send($return->order->storename->mobile, 'Return at Vegans Fresh has been approved for Order ID: '.$return->order->refid.', Product: '.($return->details->entity->name??'').', Quantity: '.$return->quantity, env('HALLO_RETURN_REQUEST_APPROVED'));
        ...
        if(isset($return->order->rider->mobile))
            Msg91::send($return->order->rider->mobile, 'Return at Vegans Fresh has been approved for Order ID: '.$return->order->refid.', Product: '.($return->details->entity->name??'').', Quantity: '.$return->quantity, env('HALLO_RETURN_REQUEST_APPROVED'));
